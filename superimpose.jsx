...
//ダブりを削る
/*
  for(var ii=1;ii<crossx.length-2;ii++){
	    for(var ii=1;ii<crossx.length-2;ii++){
  ...
  */
    ...
    for(var ii=0;ii<crossx.length-1;ii++){

      // 下から上に順番に比較します
      for(var jj=crossx.length-1;jj>ii;jj--){

	// 上の方が大きいときは互いに入れ替えます
	
	
	/*
	 	if(distance(crossx[jj],crossy[jj],ax,ay) == distance(crossx[jj-1],crossy[jj-1],ax,ay)){
	crossx= crossx.splice( jj-1, 1 ) ; 
	crossy= crossy.splice( jj-1, 1 ) ;
		}
		*/
		
	if(distance(crossx[jj],crossy[jj],ax,ay)<distance(crossx[jj-1],crossy[jj-1],ax,ay)){
	  var t=crossx[jj];
	  crossx[jj]=crossx[jj-1];
	  crossx[jj-1]=t;

	  var t=crossy[jj];
	  crossy[jj]=crossy[jj-1];
	  crossy[jj-1]=t;
	}
      }
    }
