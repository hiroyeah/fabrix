...
// 破線の場合
 if (parseFloat(activeDocument.layers[i].pathItems[j].strokeDashes) > 0) {
  ...
  if (retract>0) {
  extrudet = extrude + retract; //リトラクト
...
//      resultArrT2.push("T1");
//
 // 	   resultArrT2.push("G1 X"+x+T2x+" Y"+y+ T2y+" Z"+z+" E"+extrudetwo+" F"+h); 	 //T1T2湯浅補正あり
 // }
...
if (activeDocument.layers[i].pathItems[j].closed) {
 ...
 if (tt==1) { 
            resultArrT1.push("; ");  
            resultArrT1.push(";  path closed"); 
 } else {
	        resultArrT2.push("; ");  
            resultArrT2.push(";  path closed"); 
