...
///
     ...
     dlg.dimsPnl5 = dlg.add('panel', undefined, 'ミリ／ポイント換算 (/2.834645をする)'); 
     dlg.dimsPnl5.orientation='row';
    (dlg.dimsPnl5.selectX = dlg.dimsPnl5.add('radiobutton', [5,115,180,135], 'する' )).helpTip = ""; 
    (dlg.dimsPnl5.selectY = dlg.dimsPnl5.add('radiobutton', [5,140,180,160], 'しない' )).helpTip = "";

	dlg.dimsPnl5.selectX.value = true; 
	dlg.dimsPnl5.selectX.onClick= setTextX;
	dlg.dimsPnl5.selectY.onClick= setTextY;
...
var extln = activeDocument.layers.length;

alert(" 既存のレイヤー数 "+ extln+ "  の上に、 "+lines.length+"  行の  G-Codeを読み込みます (描画には時間がかかる場合があります)");
...
// すでにステージに同じ名前(つまりz)のレイヤーがあるかどうかチェック
var ifcheck = 0;

for (var h = 0; h < extln; h++) {

	
	if( !isNaN(activeDocument.layers[h].name)) {
 if (parseFloat(activeDocument.layers[h].name) == szzz ){
 activeDocument.activeLayer = activeDocument.layers[h]; 
 ifcheck = 1;  
 break;	 
 }
	}
